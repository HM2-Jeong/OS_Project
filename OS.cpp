...
vector<Process> parseInputFile(const string& filename) {
    ...
    string line;
    if (!file.is_open()) { // 파일 오류 체크
        cerr << "에러: " << filename << " 파일을 열 수 없습니다!\n"; 
        return processes; 
    }
    while (getline(file, line)) { // 공백이면 다음줄로 넘어가기
        if (line.empty()) continue;
        stringstream ss(line);
        Process p;
        int grade_int;
        if (!(ss >> p.id >> p.arrival_time >> grade_int >> p.service_time)) continue; // 읽어온 한 줄에서 id, 도착 시간, 등급, 서비스 시간 순서대로 데이터 추출 만약 이 순서 아닐시 버리고 다음줄
...
void resetProcesses(vector<Process>& processes) { // &를 통해 원본 수정
    for (auto& p : processes) { // 자동 형 지정, 원본 직접 가리킴, 반복문으로 초기화시킴
        p.start_time = -1;
        p.completion_time = 0;
        p.waiting_time = 0;
        p.turnaround_time = 0;
...
// =====================================================================
class ISchedulerStrategy { // 추상 클래스, 내용물 전부 순수 가상 함수
public:
...
class ProposedStrategy : public ISchedulerStrategy { // 위의 추상 클래스 구현
public:
...
private:
    Process* takeFromQ1(queue<Process*>& q) { // 도착 순서대로 선택
        if (q.empty()) return nullptr;
        Process* p = q.front(); q.pop(); 
        return p;
    }
    
    Process* takeFromQ2_HRRN(vector<Process*>& q, int current_time) { // 대기시간과 서비스 시간 계산하여 응답률이 가장 높은 사람 선택
        if (q.empty()) return nullptr;
    ...
    Process* takeFromQ3_SPN(vector<Process*>& q) { // 대기열 중 서비스 시간 가장 짧은 승객 먼저 선택
        if (q.empty()) return nullptr;
...
public:
    ...
    void assignCounters(Counter counters[], int current_time,
        ...
        for (int i = 0; i < 5; ++i) {
            if (!counters[i].is_busy) {
                Process* selected = nullptr;
                vector<Process*>* selected_q = nullptr; //3개 큐를 하나의 큰 큐로 보고 3개중 가장 먼저 온 사람부터 선택, 비선점형
    ...
    void assignCounters(Counter counters[], int current_time,
        ...
        for (int i = 0; i < 5; ++i) {
            if (!counters[i].is_busy) {
                ...
                if (selected) { 
                    // 카운터 사용중으로 변경
                    counters[i].is_busy = true; counters[i].current_process = selected; // 처리 승객 연결
                    counters[i].remaining_time = selected->service_time; 
...
public:
    Scheduler(vector<Process>& processes, ISchedulerStrategy* strat) : strategy(strat) {
        for (int i = 0; i < 5; ++i) counters[i].id = i + 1; // 카운터 번호 부여
        for (auto& p : processes) all_processes.push_back(&p); // 전체 승객 정보 확보
    }
